...
5.  **Safety First:** Prioritize accuracy, safety, and relevance in every response. Never provide harmful or inappropriate content.`;
    ...
    const handleSummarizeChat = async () => {
        ...
        try {
            ...
            if (!response.ok || !response.body) {
                const errorData = await response.json().catch(() => ({ error: 'Failed to get summary.' }));
                throw new Error(errorData.error || 'Failed to get summary.');
            }
    
            const reader = response.body.getReader();
            const decoder = new TextDecoder();
            let fullResponse = '**Chat Summary:**\n\n';
            ...
            while (true) {
                ...
                if (done) break;
                fullResponse += decoder.decode(value, { stream: true });
                await updateLastMessage({ role: 'model', text: fullResponse }, false); // Update UI without saving to DB
            }
             await updateLastMessage({ role: 'model', text: fullResponse }, true); // Save final summary to DB
    ...
    const handleStartQuiz = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Read the streaming response and assemble the full JSON
            const reader = response.body.getReader();
            const decoder = new TextDecoder();
            let jsonString = '';
            // eslint-disable-next-line no-constant-condition
